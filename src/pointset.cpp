    ...
    while ( shapes_toProcess->size() > 0 )
    {
      ...
      for ( ihs = 0; ihs < cHullSize; ihs++ )
      {
        ...
        if ( ipn != cHull[ihn] ) // next point on shape is not the next point on cHull => there is a hole here !
        {
          ...
          {
            cp = std::abs( cross_product( x[cHull[ihs]], y[cHull[ihs]],
...
#endif
          if ( seg_length < EPSILON || std::abs(( b = cross_product( ex, ey, fx, fy, x[retainedPt], y[retainedPt] ) / ( seg_length ) ) ) > ( seg_length / 2 ) )    // retainedPt is not fronting i->j
          {
