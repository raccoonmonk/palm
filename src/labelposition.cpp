...
namespace pal
{
  ...
  const std::string & LabelPosition::getLayerName() const
  {
  ...
  double LabelPosition::getDistanceToPoint( double xp, double yp )
  {
    ...
    if ( std::abs( cross_product( mx[0], my[0], mx[2], my[2], xp, yp ) / h ) < w / 2 )
    {
    ...
    if ( std::abs( cross_product( mx[1], my[1], mx[3], my[3], xp, yp ) / w ) < h / 2 )
    {
    ...
    if ( nextPart && dist_min > 0 )
      return std::min( dist_min, nextPart->getDistanceToPoint( xp, yp ) );
